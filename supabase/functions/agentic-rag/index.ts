...
class ResultReranker {
  static rerank(results: any[], query: string, maxResults: number = 5): any[] {
    ...
    const scoredResults = results.map(result => {
      ...
      // Zone match for regime urbanístico
      const zoneMatch = query.match(/ZOT\s*(\d+)/i);
      if (zoneMatch && result.Zona?.includes(zoneMatch[1])) {
        score += 0.4;
      }
      
      // Neighborhood match
      const bairroWords = ['petrópolis', 'centro', 'cidade baixa', 'moinhos'];
      bairroWords.forEach(bairro => {
        if (queryLower.includes(bairro) && 
            (result.Bairro?.toLowerCase().includes(bairro) || 
             content.includes(bairro))) {
          score += 0.3;
        }
      });
  ...
  static combineResults(legalResults: any[], regimeResults: any[], hierarchyResults: any[]): any[] {
    ...
    for (const result of combined) {
      const key = `${result.article_number || ''}${result.title || ''}${(result.content || '').substring(0, 100)}`;
      if (!seen.has(key)) {
        seen.add(key);
        unique.push(result);
      }
