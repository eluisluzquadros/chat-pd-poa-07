-- ============================================================
-- ESTRUTURA HIERÁRQUICA COMPLETA DA LUOS (VERSÃO CORRIGIDA V2)
-- Corrige erro de ambiguidade em column reference
-- ============================================================
...
-- ============================================================
-- CRIAR FUNÇÃO PARA BUSCAR HIERARQUIA DE UM ARTIGO (CORRIGIDA V2)
-- Corrige ambiguidade usando aliases explícitos
-- ============================================================
...
BEGIN
    ...
    WITH RECURSIVE article_hierarchy AS (
        ...
        SELECT 
            h.id,
            h.hierarchy_type AS h_type,
            CASE 
                WHEN h.hierarchy_number LIKE 'VI-%' THEN SUBSTRING(h.hierarchy_number FROM 4)
                ELSE h.hierarchy_number
            END as h_number,
            h.hierarchy_name AS h_name,
            h.parent_id,
            1 as level
        FROM legal_hierarchy h
        WHERE h.document_type = p_document_type
            ...
            AND h.hierarchy_type = (
                ...
                SELECT lh.hierarchy_type 
                FROM legal_hierarchy lh
                WHERE lh.document_type = p_document_type
                    AND p_article_number BETWEEN lh.article_start AND lh.article_end
                ORDER BY 
                    CASE lh.hierarchy_type
                        WHEN 'secao' THEN 1
    ...
    SELECT 
        (MAX(level) OVER() - level + 1)::INTEGER as hierarchy_level,
        h_type::VARCHAR as hierarchy_type,
        (CASE h_type
            WHEN 'titulo' THEN 'TÍTULO'
            WHEN 'capitulo' THEN 'CAPÍTULO'
            WHEN 'secao' THEN 'SEÇÃO'
            ELSE UPPER(h_type)
        END || ' ' || h_number)::TEXT as hierarchy_display,
        h_name::TEXT as hierarchy_name
    FROM article_hierarchy
    ORDER BY (MAX(level) OVER() - level + 1);
END;
$$ LANGUAGE plpgsql;

-- ============================================================
-- FUNÇÃO ALTERNATIVA: BUSCAR HIERARQUIA SIMPLES
-- Versão simplificada sem ambiguidade
-- ============================================================

CREATE OR REPLACE FUNCTION get_article_hierarchy_simple(
    p_document_type VARCHAR,
    p_article_number INTEGER
) RETURNS TEXT AS $$
DECLARE
...
BEGIN
    ...
    SELECT 
        'TÍTULO ' || 
        CASE WHEN hierarchy_number LIKE 'VI-%' THEN SUBSTRING(hierarchy_number FROM 4)
        ELSE hierarchy_number END || 
        ' - ' || hierarchy_name INTO v_rec
    FROM legal_hierarchy
    WHERE document_type = p_document_type
        AND hierarchy_type = 'titulo'
    ...
    FROM legal_hierarchy
    WHERE document_type = p_document_type
        AND hierarchy_type = 'capitulo'
        AND p_article_number BETWEEN article_start AND article_end;
    
    IF FOUND THEN
        v_result := v_result || v_rec;
    END IF;
    
    -- Buscar seção
    SELECT 
        ' > SEÇÃO ' || hierarchy_number || ' - ' || hierarchy_name INTO v_rec
    FROM legal_hierarchy
    WHERE document_type = p_document_type
        AND hierarchy_type = 'secao'
        AND p_article_number BETWEEN article_start AND article_end;
    ...
    -- Adicionar artigo
    v_result := v_result || ' > Art. ' || p_article_number || 'º';
...
-- Teste 2: Verificar hierarquia do Art. 77 usando função simples
SELECT get_article_hierarchy_simple('LUOS', 77) as hierarquia;
