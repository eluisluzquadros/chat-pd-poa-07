...
console.log('🚀 Iniciando deploy do fix do sql-generator...\n');

try {
  // Ler o arquivo da função
  const functionCode = readFileSync('./supabase/functions/sql-generator/index.ts', 'utf8');
  ...
  if (!functionCode.includes('REGRA ABSOLUTA: Se NÃO há bairro')) {
  ...
  const deployResponse = await fetch(`${supabaseUrl}/functions/v1/sql-generator`, {
    method: 'PATCH',
    headers: {
  ...
  if (!deployResponse.ok) {
    ...
    console.log('2. Encontre a função "sql-generator"');
    console.log('3. Clique em "Edit"');
    console.log('4. Cole o código atualizado de: supabase/functions/sql-generator/index.ts');
